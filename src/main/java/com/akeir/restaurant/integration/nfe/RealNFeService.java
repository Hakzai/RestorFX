...
public class RealNFeService implements NFeService {
    ...
    public NFeEmissionResult emit(NFeEmissionRequest request) {
        ...
        String protocol = "REAL-STUB-" + SEQUENCE.incrementAndGet();
        String xml = buildProviderStubXml(request, issueDate, accessKey, protocol);

        LOGGER.info("Real provider stub emission generated with access key {}", accessKey);

        return new NFeEmissionResult("PENDING_REAL_PROVIDER", accessKey, protocol, xml);
    }
    ...
    private String buildProviderStubXml(NFeEmissionRequest request, String issueDate, String accessKey, String protocol) {
        StringBuilder xml = new StringBuilder();
        xml.append("<nfeProc version=\"4.00\" provider=\"real_stub\">\n");
        xml.append("  <NFe>\n");
        xml.append("    <infNFe Id=\"NFe").append(accessKey).append("\">\n");
        xml.append("      <ide><dhEmi>").append(issueDate).append("</dhEmi></ide>\n");
        xml.append("      <emit><xNome>Restaurant Management Real Provider Stub</xNome></emit>\n");
        xml.append("      <dest>\n");
        ...
        xml.append("      <infAdic><infCpl>Awaiting real provider wiring</infCpl></infAdic>\n");
